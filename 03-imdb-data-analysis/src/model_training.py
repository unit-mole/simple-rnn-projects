"""Simple RNN training and baseline comparison."""
...
def build_simple_rnn_model(
    ...
    """Build the primary Embedding → SimpleRNN sentiment classifier."""
    import keras
    from keras import layers

    inputs = keras.Input(shape=(chunk_length,), dtype="int32", name="token_sequence")
    x = layers.Embedding(
...
def _expand_review_labels(labels: np.ndarray, review_ids: np.ndarray) -> np.ndarray:
...
def train_simple_rnn(
    ...
    """Train with a review-level split and untouched test set."""
    import keras

    required = {"review", "label"}
    for name, frame in {"train": train_frame, "test": test_frame}.items():
    ...
    tokenizer = VocabularyTokenizer(MAX_VOCABULARY).fit(train_part["review"])
    ...
    fitted = model.fit(
        x_train,
        y_train,
        validation_data=(x_validation, y_validation_chunks),
        epochs=epochs,
    ...
    x_baseline_train = baseline.fit_transform(train_frame["review"])
    x_baseline_test = baseline.transform(test_frame["review"])
    ...
    classifier.fit(x_baseline_train, train_frame["label"])
    baseline_probabilities = classifier.predict_proba(x_baseline_test)[:, 1]
